	...
	namespace library
	{
		...
		*/
		...
		/* Extracts mesh data from a .fbx file and stores it in a Mesh.
		  PARAMETERS
			_in_fbxFilepath : The filepath to the .fbx file to read from.
		...
		*/
		FBXLIB_INTERFACE Result GetMeshFromFbxFile(
			const char* _in_fbxFilepath, const char* _in_meshName,
		...
		*/
		FBXLIB_INTERFACE Result GetMaterialsFromFbxFile(
			const char* _in_fbxFilepath, uint32_t _in_materialNum,
		...
		*/
		FBXLIB_INTERFACE Result GetAnimationFromFbxFile(
			const char* _in_fbxFilepath, uint32_t _in_elementsToExtract,
